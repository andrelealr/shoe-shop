<template>
  <v-card class="mx-auto my-5" max-width="374" height="750">
    ...
    <v-divider class="mx-4"></v-divider>
        <v-card-text>
          <div class="my-4 text-subtitle-1 text-center">Antes: ${{ Number(product.precio_normal).toLocaleString('es-CL') }}</div>
          <div class="my-4 text-subtitle-1 red--text font-weight-bold text-center">Ahora ${{ Number(product.precio_promo).toLocaleString('es-CL') }}</div>
        </v-card-text>
    <v-spacer></v-spacer>
    ...
    <v-snackbar v-model="snackbar" :multi-line="multiLine" prominent shaped color="light-blue">
